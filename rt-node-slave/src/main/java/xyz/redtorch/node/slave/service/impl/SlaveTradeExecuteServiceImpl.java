...
public class SlaveTradeExecuteServiceImpl implements SlaveTradeExecuteService {
	...
	private static Logger logger = LoggerFactory.getLogger(SlaveTradeExecuteServiceImpl.class);
	...
	private Map<String, String> orderIdMap = new ConcurrentHashMap<>(5000);
	private Map<String, String> originOrderIdMap = new ConcurrentHashMap<>(5000);
	...
	public String submitOrder(CommonReqField commonReq, SubmitOrderReqField submitOrderReq) {
		...
		if (submitOrderReq == null) {
			...
			orderIdMap.put(submitOrderReq.getOriginOrderId(), orderId);
			originOrderIdMap.put(orderId, submitOrderReq.getOriginOrderId());
	...
	public boolean cancelOrder(CommonReqField commonReq, CancelOrderReqField cancelOrderReq) {
		...
		if (cancelOrderReq == null) {
			...
			throw new IllegalArgumentException("撤销定单错误，参数orderId与originOrderId同时缺失");
				...
				&& StringUtils.isBlank(orderIdMap.get(cancelOrderReq.getOriginOrderId()))) {
			...
			if (StringUtils.isBlank(orderId)) {
				orderId = orderIdMap.get(cancelOrderReq.getOriginOrderId());
			}
			...
			if (submitOrderReq == null) {
				...
				if (order == null) {
					...
					String gatewayId = order.getContract().getGateway().getGatewayId();
				...
				if (gatewayApi == null) {
					...
					boolean result = gatewayApi.cancelOrder(cancelOrderReq);
					logger.info("撤销定单已经执行,原始定单ID:{},定单ID:{},网关返回结果:{}", submitOrderReq.getOriginOrderId(), orderId,
							result);
					return result;
				}
			}
	...
	public void searchContract(CommonReqField commonReq, ContractField contract) {
		...
		} else if (contract.getGateway() == null) {
			logger.error("搜索合约错误,网关为空");
			throw new IllegalArgumentException("搜索合约错误,网关为空");
		} else if (StringUtils.isBlank(contract.getGateway().getGatewayId())) {
			logger.error("搜索合约错误,网关ID为空");
			throw new IllegalArgumentException("搜索合约错误,网关ID为空");
		} else if (slaveSystemService.getGatewayApi(contract.getGateway().getGatewayId()) == null) {
			logger.error("搜索合约错误,网关不存在,网关ID:{}", contract.getGateway().getGatewayId());
			throw new RuntimeException("搜索合约错误,网关不存在,网关ID:" + contract.getGateway().getGatewayId());
		} else {
			logger.info("搜索合约请求记录,合约:{}", contract.toString());
			GatewayApi gatewayApi = slaveSystemService.getGatewayApi(contract.getGateway().getGatewayId());
