...
class Center extends PureComponent {
  ...
  render() {
    ...
    const columns = [{
      ...
      render:(text,record)=>(
        <div className={` ${styles.displayRight}`}>
          <div><span style={INLINE_LABEL_STYLE}>持仓：</span>{numberFormat(record.price,4)}</div>
      ...
      render:(text,record)=>(
        <div className={`${styles.displayRight}`}>
          <div><span style={INLINE_LABEL_STYLE}>持仓：</span>{numberFormat(record.priceDiff,4)}</div>
        ...
        if(record.openProfit > 0){
          return(
            <div className={`${styles.displayRight} ${styles.colorBuy}`}>
              <div>{numberFormat(record.openProfit,4)}</div>
      ...
      render:(text,record)=>{
        if( record.positionProfit > 0){
          return(
            <div className={`${styles.displayRight} ${styles.colorBuy}`}>
              <div>{numberFormat( record.positionProfit,4)}</div>
      ...
      render:(text,record)=>(
        <div className={`${styles.displayRight}`}>
          <div><span style={INLINE_LABEL_STYLE}>经济商：</span>{numberFormat( record.useMargin,2)}</div>
