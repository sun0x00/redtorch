        ...
        init {
            ...
            try {
                if (System.getProperties().getProperty("os.name").uppercase(Locale.getDefault()).contains("WINDOWS")) {
                    ...
                    tempLibPath =
                        "${envTmpDir}${separator}xyz${separator}redtorch${separator}api${separator}jctp${separator}lib${separator}jctpv6v6v9x64api${separator}"

                    copyFileByURL(tempLibPath, CtpGatewayImpl::class.java.getResource("/assembly/libiconv.dll")!!)
                    copyFileByURL(
                        tempLibPath,
                        CtpGatewayImpl::class.java.getResource("/assembly/jctpv6v6v9x64api/thostmduserapi_se.dll")!!
                    )
                    copyFileByURL(
                        tempLibPath,
                        CtpGatewayImpl::class.java.getResource("/assembly/jctpv6v6v9x64api/thosttraderapi_se.dll")!!
                    )
                    copyFileByURL(
                        tempLibPath,
                        CtpGatewayImpl::class.java.getResource("/assembly/jctpv6v6v9x64api/jctpv6v6v9x64api.dll")!!
                    )
                } else {
                    envTmpDir = "/tmp"
                    val separator = File.separator
                    ...
                    copyFileByURL(
                        tempLibPath,
                        CtpGatewayImpl::class.java.getResource("/assembly/jctpv6v6v9x64api/libthostmduserapi_se.so")!!
                    )
                    copyFileByURL(
                        tempLibPath,
                        CtpGatewayImpl::class.java.getResource("/assembly/jctpv6v6v9x64api/libthosttraderapi_se.so")!!
                    )
                    copyFileByURL(
                        tempLibPath,
                        CtpGatewayImpl::class.java.getResource("/assembly/jctpv6v6v9x64api/libjctpv6v6v9x64api.so")!!
                    )
                }
            } catch (e: Exception) {
            ...
            try {
                if (System.getProperties().getProperty("os.name").uppercase(Locale.getDefault()).contains("WINDOWS")) {
                    ...
                    System.load("$tempLibPath${File.separator}jctpv6v6v9x64api.dll")
                } else {
                    ...
                    System.load("$tempLibPath${File.separator}libjctpv6v6v9x64api.so")
                }
            } catch (e: Exception) {
