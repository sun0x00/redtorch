...
public class OperatorServiceImpl implements OperatorService, InitializingBean {
	...
	public boolean checkSubscribePermission(String operatorId, String unifiedSymbol) {
		...
		if (adminOperatorId.equals(operatorId)) {
			...
			if (operator != null && operator.isCanSubscribeAllContracts()
	...
	public boolean checkReadAccountPermission(String operatorId, String accountId) {
		...
		if (adminOperatorId.equals(operatorId)) {
			...
			OperatorPo operator = getOperatorByOperatorId(operatorId);
			if (operator != null && operator.isCanReadAllAccounts()
					&& !operator.getDenyReadSpecialAccountIdSet().contains(accountId)) {
				canReadAccount = true;
			} else if (operator != null && operator.getAcceptReadSpecialAccountIdSet().contains(accountId)
					&& !operator.getDenyReadSpecialAccountIdSet().contains(accountId)) {
				canReadAccount = true;
			}
		}
	...
	public boolean checkTradeAccountPermission(String operatorId, String accountId) {
		...
		if (adminOperatorId.equals(operatorId)) {
			...
			OperatorPo operator = getOperatorByOperatorId(operatorId);
			if (operator != null && operator.isCanTradeAllAccounts()
					&& !operator.getDenyTradeSpecialAccountIdSet().contains(accountId)) {
				canTradeAccount = true;
			} else if (operator != null && operator.getAcceptTradeSpecialAccountIdSet().contains(accountId)
					&& !operator.getDenyTradeSpecialAccountIdSet().contains(accountId)) {
				canTradeAccount = true;
			}
		}
	...
	public boolean checkTradeContractPermission(String operatorId, String unifiedSymbol) {
		...
		if (adminOperatorId.equals(operatorId)) {
			...
			if (operator != null && operator.isCanTradeAllContracts()
					&& !operator.getDenyTradeSpecialUnifiedSymbolSet().contains(unifiedSymbol)) {
				canTradeContract = true;
			} else if (operator != null && operator.getAcceptTradeSpecialUnifiedSymbolSet().contains(unifiedSymbol)
					&& !operator.getDenyTradeSpecialUnifiedSymbolSet().contains(unifiedSymbol)) {
				canTradeContract = true;
			}
