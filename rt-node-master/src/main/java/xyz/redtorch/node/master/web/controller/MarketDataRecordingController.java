...
public class MarketDataRecordingController {
	private static Logger logger = LoggerFactory.getLogger(MarketDataRecordingController.class);
	...
	public ResponseVo<String> addContractByUnifiedSymbol(HttpServletRequest request, @RequestBody ContractPo contract) {
		...
		try {
			...
			if (user.isCanWriteMarketDataRecording()) {
				logger.info("用户{}新增合约,合约统一标识:{}", user.getUsername(), contract.getUnifiedSymbol());
				marketDataRecordingService.addContractByUnifiedSymbol(contract);
			} else {
