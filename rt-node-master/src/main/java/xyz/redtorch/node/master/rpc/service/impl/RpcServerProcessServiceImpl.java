...
public class RpcServerProcessServiceImpl implements RpcServerProcessService, InitializingBean {
	...
	private ExecutorService unimportantFixedExecutorService = Executors.newFixedThreadPool(1);
	private ExecutorService tradeRtnQueueFixedExecutorService = Executors.newFixedThreadPool(1);
	private ExecutorService marketRtnQueueFixedExecutorService = Executors.newFixedThreadPool(1);
	...
	public void processData(int sourceNodeId, byte[] data) {
		...
		if (targetNodeId != 0) {
			...
			logger.info("转发DEP记录,来源节点ID:{},目标节点ID：{},RPC类型:{},RPC ID:{},请求ID:{},内容类型:{},时间戳:{}", sourceNodeId,
					targetNodeId, sourceNodeId, rpcTypeValueName, rpcId, reqId, contentTypeValueName, timestamp);

			if (webSocketServerHandler.sendDataByNodeId(targetNodeId, data)) {
				logger.error("转发DEP错误,来源节点ID:{},目标节点ID：{},RPC类型:{},RPC ID:{},请求ID:{},内容类型:{},时间戳:{}", sourceNodeId,
						targetNodeId, sourceNodeId, rpcTypeValueName, rpcId, reqId, contentTypeValueName, timestamp);
			}

		} else {

			logger.info("处理DEP记录,来源节点ID:{},RPC类型:{},RPC ID:{},请求ID:{},内容类型:{},时间戳:{}", sourceNodeId, rpcTypeValueName,
					rpcId, reqId, contentTypeValueName, timestamp);
			...
			if (contentType == ContentType.COMPRESSED_LZ4) {
				try (InputStream in = new ByteArrayInputStream(dep.getContentBytes().toByteArray());
					...
					logger.error("处理DEP错误,来源节点ID:{},RPC类型:{},RPC ID:{},请求ID:{},时间戳:{},无法使用LZ4正确解析报文内容", sourceNodeId,
							sourceNodeId, rpcTypeValueName, rpcId, timestamp, e);
				...
				logger.error("处理DEP错误,来源节点ID:{},RPC类型:{},RPC ID:{},请求ID:{},内容类型:{},时间戳:{},不支持的报文类型", sourceNodeId,
						sourceNodeId, rpcTypeValueName, rpcId, reqId, contentTypeValueName, timestamp);
				sendExceptionRsp(sourceNodeId, rpcId, reqId, timestamp, "不支持的报文类型");
				return;
			}

			if (contentByteString == null || contentByteString.size() <= 0) {
				logger.error("处理DEP错误,来源节点ID:{},RPC类型:{},RPC ID:{},请求ID:{},时间戳:{},报文内容长度错误", sourceNodeId, sourceNodeId,
						rpcTypeValueName, rpcId, reqId, timestamp);
			...
			if (dep.getRpcType() == RpcType.CORE_RPC) {
				...
				logger.error("处理DEP错误,来源节点ID:{},RPC类型:{},RPC ID:{},请求ID:{},时间戳:{},未能识别的RPC类型", sourceNodeId,
						dep.getRpcType().getValueDescriptor().getName(), rpcId, reqId, timestamp);
			}
		}
	}
	...
	private void doCoreRpc(int sourceNodeId, int rpcId, String reqId, ByteString contentByteString, long timestamp) {
			...
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						rpcServerReqHandlerService.subscribe(rpcSubscribeReq.getCommonReq(),
								rpcSubscribeReq.getContract(), rpcSubscribeReq.getGatewayId());
					} catch (Exception e) {
			...
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						rpcServerReqHandlerService.unsubscribe(rpcUnsubscribeReq.getCommonReq(),
								rpcUnsubscribeReq.getContract(), rpcUnsubscribeReq.getGatewayId());
					} catch (Exception e) {
			...
			importantExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						rpcServerReqHandlerService.submitOrder(rpcSubmitOrderReq.getCommonReq(),
			...
			importantExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						rpcServerReqHandlerService.cancelOrder(rpcCancelOrderReq.getCommonReq(),
			...
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						rpcServerReqHandlerService.searchContract(rpcSearchContractReq.getCommonReq(),
		...
		case RpcId.GET_WORKING_ORDER_LIST_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcGetWorkingOrderListReq rpcGetWorkingOrderListReq = RpcGetWorkingOrderListReq
		...
		case RpcId.QUERY_ORDER_BY_ORDER_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryOrderByOrderIdReq rpcQueryOrderByOrderIdReq = RpcQueryOrderByOrderIdReq
								.parseFrom(contentByteString);
						...
						rpcServerReqHandlerService.queryOrderByOrderId(rpcQueryOrderByOrderIdReq.getCommonReq(),
								rpcQueryOrderByOrderIdReq.getOrderId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_ORDER_BY_ORIGIN_ORDER_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryOrderByOriginOrderIdReq rpcQueryOrderByOriginOrderIdReq = RpcQueryOrderByOriginOrderIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryOrderByOriginOrderIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_ORDER_BY_ORIGIN_ORDER_ID_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.queryOrderByOriginOrderId(
								rpcQueryOrderByOriginOrderIdReq.getCommonReq(),
								rpcQueryOrderByOriginOrderIdReq.getOriginOrderId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_ORDER_LIST_BY_ACCOUNT_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryOrderListByAccountIdReq rpcQueryOrderListByAccountIdReq = RpcQueryOrderListByAccountIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryOrderListByAccountIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_ORDER_LIST_BY_ACCOUNT_ID_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.queryOrderListByAccountId(
								rpcQueryOrderListByAccountIdReq.getCommonReq(),
								rpcQueryOrderListByAccountIdReq.getAccountId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_ORDER_LIST_BY_UNIFIED_SYMBOL_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryOrderListByUnifiedSymbolReq rpcQueryOrderListByUnifiedSymbolReq = RpcQueryOrderListByUnifiedSymbolReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryOrderListByUnifiedSymbolReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_ORDER_LIST_BY_UNIFIED_SYMBOL_REQ",
								sourceNodeId, reqId);
						rpcServerReqHandlerService.queryOrderListByUnifiedSymbol(
								rpcQueryOrderListByUnifiedSymbolReq.getCommonReq(),
								rpcQueryOrderListByUnifiedSymbolReq.getUnifiedSymbol());
					} catch (Exception e) {
		...
		case RpcId.QUERY_TRADE_BY_TRADE_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryTradeByTradeIdReq rpcQueryTradeByTradeIdReq = RpcQueryTradeByTradeIdReq
								.parseFrom(contentByteString);
						...
						rpcServerReqHandlerService.queryTradeByTradeId(rpcQueryTradeByTradeIdReq.getCommonReq(),
								rpcQueryTradeByTradeIdReq.getTradeId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_TRADE_LIST_BY_UNIFIED_SYMBOL_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryTradeListByUnifiedSymbolReq rpcQueryTradeListByUnifiedSymbolReq = RpcQueryTradeListByUnifiedSymbolReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryTradeListByUnifiedSymbolReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_TRADE_LIST_BY_UNIFIED_SYMBOL_REQ",
								sourceNodeId, reqId);
						rpcServerReqHandlerService.queryTradeListByUnifiedSymbol(
								rpcQueryTradeListByUnifiedSymbolReq.getCommonReq(),
								rpcQueryTradeListByUnifiedSymbolReq.getUnifiedSymbol());
					} catch (Exception e) {
		...
		case RpcId.QUERY_TRADE_LIST_BY_ACCOUNT_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryTradeListByAccountIdReq rpcQueryTradeListByAccountIdReq = RpcQueryTradeListByAccountIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryTradeListByAccountIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_TRADE_LIST_BY_ACCOUNT_ID_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.queryTradeListByAccountId(
								rpcQueryTradeListByAccountIdReq.getCommonReq(),
								rpcQueryTradeListByAccountIdReq.getAccountId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_TRADE_LIST_BY_ORDER_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryTradeListByOrderIdReq rpcQueryTradeListByOrderIdReq = RpcQueryTradeListByOrderIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryTradeListByOrderIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_TRADE_LIST_BY_ORDER_ID_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.queryTradeListByOrderId(rpcQueryTradeListByOrderIdReq.getCommonReq(),
								rpcQueryTradeListByOrderIdReq.getOrderId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_TRADE_LIST_BY_ORIGIN_ORDER_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryTradeListByOriginOrderIdReq rpcQueryTradeListByOriginOrderIdReq = RpcQueryTradeListByOriginOrderIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryTradeListByOriginOrderIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_TRADE_LIST_BY_ORIGIN_ORDER_ID_REQ",
								sourceNodeId, reqId);
						rpcServerReqHandlerService.queryTradeListByOriginOrderId(
								rpcQueryTradeListByOriginOrderIdReq.getCommonReq(),
								rpcQueryTradeListByOriginOrderIdReq.getOriginOrderId());
					} catch (Exception e) {
		...
		case RpcId.GET_POSITION_LIST_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcGetPositionListReq rpcGetPositionListReq = RpcGetPositionListReq
								.parseFrom(contentByteString);
		...
		case RpcId.QUERY_POSITION_BY_POSITION_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryPositionByPositionIdReq rpcQueryPositionByPositionIdReq = RpcQueryPositionByPositionIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryPositionByPositionIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_POSITION_BY_POSITION_ID_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.queryPositionByPositionId(
								rpcQueryPositionByPositionIdReq.getCommonReq(),
								rpcQueryPositionByPositionIdReq.getPositionId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_POSITION_LIST_BY_ACCOUNT_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryPositionListByAccountIdReq rpcQueryPositionListByAccountIdReq = RpcQueryPositionListByAccountIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryPositionListByAccountIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_POSITION_LIST_BY_ACCOUNT_ID_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.queryPositionListByAccountId(
								rpcQueryPositionListByAccountIdReq.getCommonReq(),
								rpcQueryPositionListByAccountIdReq.getAccountId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_POSITION_LIST_BY_UNIFIED_SYMBOL_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryPositionListByUnifiedSymbolReq rpcQueryPositionListByUnifiedSymbolReq = RpcQueryPositionListByUnifiedSymbolReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryPositionListByUnifiedSymbolReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_POSITION_LIST_BY_UNIFIED_SYMBOL_REQ",
								sourceNodeId, reqId);
						rpcServerReqHandlerService.queryPositionListByUnifiedSymbol(
								rpcQueryPositionListByUnifiedSymbolReq.getCommonReq(),
								rpcQueryPositionListByUnifiedSymbolReq.getUnifiedSymbol());
					} catch (Exception e) {
						logger.error("处理RPC异常,来源节点ID:{},RPC:QUERY_POSITION_LIST_BY_UNIFIED_SYMBOL_REQ", sourceNodeId,
								e);
						sendExceptionRsp(sourceNodeId, rpcId, reqId, timestamp, e.getMessage());
					}
				}
		...
		case RpcId.QUERY_ACCOUNT_BY_ACCOUNT_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryAccountByAccountIdReq rpcQueryAccountByAccountIdReq = RpcQueryAccountByAccountIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryAccountByAccountIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_ACCOUNT_BY_ACCOUNT_ID_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.queryAccountByAccountId(rpcQueryAccountByAccountIdReq.getCommonReq(),
								rpcQueryAccountByAccountIdReq.getAccountId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_ACCOUNT_LIST_BY_ACCOUNT_CODE_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryAccountListByAccountCodeReq rpcQueryAccountListByAccountCodeReq = RpcQueryAccountListByAccountCodeReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryAccountListByAccountCodeReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_ACCOUNT_LIST_BY_ACCOUNT_CODE_REQ",
								sourceNodeId, reqId);
						rpcServerReqHandlerService.queryAccountListByAccountCode(
								rpcQueryAccountListByAccountCodeReq.getCommonReq(),
								rpcQueryAccountListByAccountCodeReq.getAccountCode());
					} catch (Exception e) {
		...
		case RpcId.GET_CONTRACT_LIST_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcGetContractListReq rpcGetContractListReq = RpcGetContractListReq
								.parseFrom(contentByteString);
		...
		case RpcId.QUERY_CONTRACT_BY_CONTRACT_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryContractByContractIdReq rpcQueryContractByContractIdReq = RpcQueryContractByContractIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryContractByContractIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_CONTRACT_BY_CONTRACT_ID_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.queryContractByContractId(
								rpcQueryContractByContractIdReq.getCommonReq(),
								rpcQueryContractByContractIdReq.getContractId());
					} catch (Exception e) {
		...
		case RpcId.QUERY_CONTRACT_LIST_BY_UNIFIED_SYMBOL_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryContractListByUnifiedSymbolReq rpcQueryContractListByUnifiedSymbolReq = RpcQueryContractListByUnifiedSymbolReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryContractListByUnifiedSymbolReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_CONTRACT_LIST_BY_UNIFIED_SYMBOL_REQ",
								sourceNodeId, reqId);
						rpcServerReqHandlerService.queryContractListByUnifiedSymbol(
								rpcQueryContractListByUnifiedSymbolReq.getCommonReq(),
								rpcQueryContractListByUnifiedSymbolReq.getUnifiedSymbol());
					} catch (Exception e) {
						logger.error("处理RPC异常,来源节点ID:{},RPC:QUERY_CONTRACT_LIST_BY_UNIFIED_SYMBOL_REQ", sourceNodeId,
								e);
						sendExceptionRsp(sourceNodeId, rpcId, reqId, timestamp, e.getMessage());
					}
				}
		...
		case RpcId.QUERY_CONTRACT_LIST_BY_GATEWAY_ID_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcQueryContractListByGatewayIdReq rpcQueryContractListByGatewayIdReq = RpcQueryContractListByGatewayIdReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcQueryContractListByGatewayIdReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:QUERY_CONTRACT_LIST_BY_GATEWAY_ID_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.queryContractListByGatewayId(
								rpcQueryContractListByGatewayIdReq.getCommonReq(),
								rpcQueryContractListByGatewayIdReq.getGatewayId());
					} catch (Exception e) {
		...
		case RpcId.SYNC_SLAVE_NODE_RUNTIME_DATA_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcSyncSlaveNodeRuntimeDataReq rpcSyncSlaveNodeRuntimeDataReq = RpcSyncSlaveNodeRuntimeDataReq
								.parseFrom(contentByteString);
						checkCommonReq(rpcSyncSlaveNodeRuntimeDataReq.getCommonReq(), sourceNodeId, reqId);
						logger.info("处理RPC记录,来源节点ID:{},请求ID:{},RPC:SYNC_SLAVE_NODE_RUNTIME_DATA_REQ", sourceNodeId,
								reqId);
						rpcServerReqHandlerService.syncSlaveNodeRuntimeData(
								rpcSyncSlaveNodeRuntimeDataReq.getCommonReq(),
								rpcSyncSlaveNodeRuntimeDataReq.getGatewayList());
					} catch (Exception e) {
		...
		case RpcId.GET_MIX_CONTRACT_LIST_REQ_VALUE: {
			normalExecutorService.execute(new Runnable() {
				...
				public void run() {
					...
					try {
						RpcGetMixContractListReq rpcGetMixContractListReq = RpcGetMixContractListReq
								.parseFrom(contentByteString);
		...
		case RpcId.ORDER_RTN_VALUE: {
			tradeRtnQueueFixedExecutorService.execute(new Runnable() {
				@Override
				public void run() {
					try {
		...
		case RpcId.CONTRACT_RTN_VALUE: {
			unimportantFixedExecutorService.execute(new Runnable() {
				@Override
				public void run() {
		...
		case RpcId.TICK_RTN_VALUE: {
			marketRtnQueueFixedExecutorService.execute(new Runnable() {
				@Override
				public void run() {
					try {
	...
	private void checkCommonRtn(CommonRtnField commonRtn, int sourceNodeId) {
		...
		if (commonRtn.getTargetNodeId() != 0) {
	...
	private void checkCommonRsp(CommonRspField commonRsp, int sourceNodeId, String reqId) {
		...
		if (commonRsp.getTargetNodeId() != 0) {
			throw new IllegalArgumentException("目标节点ID不为0!");
		}
