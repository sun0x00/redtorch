...
public class GatewayServiceImpl implements GatewayService, InitializingBean {
				...
				for (GatewayPo gateway : gatewayList) {
					if (gateway.getStatus() == ConnectStatusEnum.CONNECTED_VALUE) {
						if (!nodeMap.containsKey(gateway.getTargetNodeId())
								|| nodeMap.get(gateway.getTargetNodeId()).getStatus() != NodePo.NODE_STATUS_CONNECTED) {
	...
	public void deleteGatewayByGatewayId(String gatewayId) {
		...
		masterTradeCachesService.removeAllCachesByGatewayId(gatewayId);
	}
	...
	public void connectGatewayByGatewayId(String gatewayId) {
		...
		if (gateway.getStatus() == ConnectStatusEnum.DISCONNECTING_VALUE
				|| gateway.getStatus() == ConnectStatusEnum.DISCONNECTED_VALUE || gateway.getStatus() == null) {
			gateway.setStatus(ConnectStatusEnum.CONNECTING_VALUE);
	...
	public void disconnectGatewayByGatewayId(String gatewayId) {
		...
		if (gateway.getStatus() == ConnectStatusEnum.CONNECTING_VALUE
				|| gateway.getStatus() == ConnectStatusEnum.CONNECTED_VALUE || gateway.getStatus() == null) {
	...
	public void disconnectAllGateways() {
		...
		for (GatewayPo gateway : gatewayList) {
			...
			masterTradeCachesService.removeAllCachesByGatewayId(gateway.getGatewayId());
		}
	}
