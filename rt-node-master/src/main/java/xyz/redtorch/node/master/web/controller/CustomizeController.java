...
public class CustomizeController {
	...
	@ResponseBody
	public ResponseVo<String> addFavoriteContractByUnifiedSymbol(HttpServletRequest request,
			@RequestBody ContractPo contract) {
		ResponseVo<String> responseVo = new ResponseVo<>();
		try {
			UserPo user = (UserPo) request.getSession().getAttribute(RtConstant.KEY_USER_PO);
			favoriteContractService.upsertContractByUsernameAndUnifiedSymbol(user.getUsername(), contract);
		} catch (Exception e) {
	...
	@ResponseBody
	public ResponseVo<String> deleteFavoriteContractByUnifiedSymbol(HttpServletRequest request,
