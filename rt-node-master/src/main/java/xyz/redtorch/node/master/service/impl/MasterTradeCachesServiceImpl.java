...
public class MasterTradeCachesServiceImpl implements MasterTradeCachesService, InitializingBean {
	...
	private static Logger logger = LoggerFactory.getLogger(MasterTradeCachesServiceImpl.class);
		...
		scheduledExecutorService.scheduleAtFixedRate(() -> {
			try {
				...
				for (GatewayPo gateway : gatewayList) {
					if (ConnectStatusEnum.CONNECTED_VALUE == gateway.getStatus()
							|| ConnectStatusEnum.CONNECTING_VALUE == gateway.getStatus()) {
				...
				try {
					...
					accountMap = accountMap.entrySet().stream()
							.filter(map -> retainGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
	...
	public List<OrderField> getOrderList(String operatorId) {
		...
		try {
			orderList = orderMap.values().stream()
					.filter(order -> operatorService.checkReadAccountPermission(operatorId, order.getAccountId()))
	...
	public List<OrderField> getWorkingOrderList(String operatorId) {
		...
		orderMapLock.lock();
		try {
			orderList = workingOrderMap.values().stream()
					.filter(order -> operatorService.checkReadAccountPermission(operatorId, order.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<OrderField> queryOrderListByAccountId(String operatorId, String accountId) {
		...
		try {
			if (operatorService.checkReadAccountPermission(operatorId, accountId)) {
				orderList = orderMap.values().stream().filter(order -> order.getAccountId().equals(accountId))
						.collect(Collectors.toList());
			}
	...
	public List<OrderField> queryOrderListByUnifiedSymbol(String operatorId, String unifiedSymbol) {
		...
		try {
			orderList = orderMap.values().stream()
					.filter(order -> order.getContract().getUnifiedSymbol().equals(unifiedSymbol)
							&& operatorService.checkReadAccountPermission(operatorId, order.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<TradeField> getTradeList(String operatorId) {
		...
		try {
			tradeList = tradeMap.values().stream()
					.filter(trade -> operatorService.checkReadAccountPermission(operatorId, trade.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<TradeField> queryTradeListByUnifiedSymbol(String operatorId, String unifiedSymbol) {
		...
		try {
			tradeList = tradeMap.values().stream()
					.filter(trade -> trade.getContract().getUnifiedSymbol().equals(unifiedSymbol)
							&& operatorService.checkReadAccountPermission(operatorId, trade.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<TradeField> queryTradeListByOriginOrderId(String operatorId, String originOrderId) {
		...
		try {
			tradeList = tradeMap.values().stream()
					.filter(trade -> trade.getOriginOrderId().equals(originOrderId)
							&& operatorService.checkReadAccountPermission(operatorId, trade.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<PositionField> getPositionList(String operatorId) {
		...
		try {
			positionList = positionMap.values().stream()
					.filter(position -> operatorService.checkReadAccountPermission(operatorId, position.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<PositionField> queryPositionListByAccountId(String operatorId, String accountId) {
		...
		try {
			if (operatorService.checkReadAccountPermission(operatorId, accountId)) {
				positionList = positionMap.values().stream()
						.filter(positionField -> positionField.getAccountId().equals(accountId))
						.collect(Collectors.toList());
			}
	...
	public List<PositionField> queryPositionListByUnifiedSymbol(String operatorId, String unifiedSymbol) {
		...
		try {
			positionList = positionMap.values().stream()
					.filter(position -> position.getContract().getUnifiedSymbol().equals(unifiedSymbol)
							&& operatorService.checkReadAccountPermission(operatorId, position.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<AccountField> getAccountList(String operatorId) {
		...
		accountMapLock.lock();
		try {
			accountList = accountMap.values().stream()
					.filter(account -> operatorService.checkReadAccountPermission(operatorId, account.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
		...
		try {
			accountList = accountMap.values().stream()
					.filter(account -> account.getCode().equals(accountCode)
							&& operatorService.checkReadAccountPermission(operatorId, account.getAccountId()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<TickField> getTickList(String operatorId) {
		...
		tickMapLock.lock();
		try {
			tickList = tickMap.values().stream().filter(
					tick -> operatorService.checkSubscribePermission(operatorId, tick.getContract().getUnifiedSymbol()))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	public List<ContractField> queryContractListByGatewayId(String operatorId, String gatewayId) {
		...
		try {
			contractList = contractMap.values().stream()
					.filter(contractField -> contractField.getGateway().getGatewayId().equals(gatewayId))
					.collect(Collectors.toList());
		} catch (Exception e) {
	...
	@Override
	public void cacheOrderList(List<OrderField> orderList, int sourceNodeId) {
		orderMapLock.lock();
		try {
			...
			orderMap = orderMap.entrySet().stream()
					.filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
	...
	@Override
	public void cacheTradeList(List<TradeField> tradeList, int sourceNodeId) {
		tradeMapLock.lock();
		try {
			Set<String> removeGatewayIdSet = new HashSet<>();
			List<GatewayPo> gatewayList = gatewayDao.queryGatewayList();
	...
	@Override
	public void cacheContractList(List<ContractField> contractList, int sourceNodeId) {
		contractMapLock.lock();
		try {
			Set<String> removeGatewayIdSet = new HashSet<>();
			List<GatewayPo> gatewayList = gatewayDao.queryGatewayList();
			...
			contractMap = contractMap.entrySet().stream()
					.filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
			mixContractMap = mixContractMap.entrySet().stream()
					.filter(map -> !removeGatewayIdSet.contains(map.getValue().getGateway().getGatewayId()))
					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
	...
	@Override
	public void cachePositionList(List<PositionField> positionList, int sourceNodeId) {
		positionMapLock.lock();
		try {
			Set<String> removeGatewayIdSet = new HashSet<>();
			List<GatewayPo> gatewayList = gatewayDao.queryGatewayList();
	...
	@Override
	public void cacheAccountList(List<AccountField> accountList, int sourceNodeId) {
		accountMapLock.lock();
		try {
			Set<String> removeGatewayIdSet = new HashSet<>();
	...
	@Override
	public void cacheTickList(List<TickField> tickList, int sourceNodeId) {
		tickMapLock.lock();
		try {
			Set<String> removeGatewayIdSet = new HashSet<>();
