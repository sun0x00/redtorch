...
public class XMinBarGenerator {
	...
	public void updateBar(BarField bar) {
		...
		// 如果已经不属于同一个周期
		if (xMinBarBuilder != null && (int) xMinBarLocalDateTime.get(ChronoField.MINUTE_OF_DAY)
				/ xMin != (int) barLocalDateTime.get(ChronoField.MINUTE_OF_DAY) / xMin) {
			finish();
		}
	...
	public void finish() {
		if (xMinBarLocalDateTime != null && xMinBarBuilder != null) {
			xMinBarLocalDateTime = xMinBarLocalDateTime
					.minusMinutes(xMinBarLocalDateTime.get(ChronoField.MINUTE_OF_DAY) % xMin).withSecond(0).withNano(0);
