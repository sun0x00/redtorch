...
public class XMinBarGenerator {
    ...
    public void updateBar(BarField bar) {
        ...
        // 如果交易日发生了变更
        if (xMinBarBuilder != null && xMinBarBuilder.getTradingDay() != bar.getTradingDay()) {
            finish();
        }

        // 如果日期发生了变更
        if (xMinBarBuilder != null && xMinBarBuilder.getActionDay() != bar.getActionDay()) {
            finish();
        }
        ...
        if (xMinBarBuilder == null) {
            ...
            xMinBarBuilder.setPreSettlePrice(bar.getPreSettlePrice());
            xMinBarBuilder.setPreClosePrice(bar.getPreClosePrice());
            xMinBarBuilder.setPreOpenInterest(bar.getPreOpenInterest());
    ...
    public void finish() {
        if (xMinBarLocalDateTime != null && xMinBarBuilder != null) {
            ...
            xMinBarBuilder.setActionTime(Integer.parseInt(xMinBarLocalDateTime.format(CommonConstant.T_FORMAT_WITH_MS_INT_FORMATTER)));
            xMinBarBuilder.setActionTimestamp(CommonUtils.localDateTimeToMills(xMinBarLocalDateTime));

            // 回调onXMinBar方法
            commonBarCallBack.call(xMinBarBuilder);
        }
