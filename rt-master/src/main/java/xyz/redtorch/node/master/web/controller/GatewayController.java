...
@Controller
@RequestMapping("${rt.master.apiBasePath}/management/gateway")
			...
			if (user.isCanReadGateway()) {
				...
				for (GatewayPo gateway : gatewayList) {
					if (gateway.getCtpSetting() != null) {
						gateway.getCtpSetting().setAuthCode(CommonConstant.SECURITY_MASK);
						gateway.getCtpSetting().setPassword(CommonConstant.SECURITY_MASK);
					}
