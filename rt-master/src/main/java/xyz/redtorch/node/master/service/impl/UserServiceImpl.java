...
public class UserServiceImpl implements UserService {
    ...
    public UserPo auth(UserPo user) {
        ...
        if (user.getUsername().equals("admin")) {
            ...
            if (adminPasswordSha256.equals(passwordSha256hex)) {
                UserPo retUser = new UserPo();
                retUser.setUsername("admin");
                logger.error("用户审核通过,用户名:{}", user.getUsername());
                retUser.setOperatorId(masterOperatorId);
                // 全部授权
                retUser.setCanChangeGatewayStatus(true);
                retUser.setCanChangeNodeToken(true);
                retUser.setCanChangeOperatorStatus(true);
                retUser.setCanReadGateway(true);
                retUser.setCanReadLog(true);
                retUser.setCanReadMarketDataRecording(true);
                retUser.setCanReadNode(true);
                retUser.setCanReadOperator(true);
                retUser.setCanReadUser(true);
                retUser.setCanWriteGateway(true);
                retUser.setCanWriteMarketDataRecording(true);
                retUser.setCanWriteNode(true);
                retUser.setCanWriteOperator(true);
                retUser.setCanWriteUser(true);
                return retUser;
            }
        } else {
            // 非admin用户查询数据库
            ...
            if (queriedUser != null && !StringUtils.isBlank(queriedUser.getPassword())) {
                if (queriedUser.getPassword().equals(passwordSha256hex)) {
                    ...
                    queriedUser.setRecentlyLoginTime(CommonConstant.DT_FORMAT_WITH_MS_FORMATTER.format(LocalDateTime.now()));
    ...
    public void deleteUserByUsername(String username) {
        ...
        haSessionMangerService.removeSessionByUsername(username);
