...
import xyz.redtorch.gateway.ctp.x64v6v3v16v.api.CThostFtdcDepthMarketDataField;
import xyz.redtorch.gateway.ctp.x64v6v3v16v.api.CThostFtdcForQuoteRspField;
import xyz.redtorch.gateway.ctp.x64v6v3v16v.api.CThostFtdcMdApi;
import xyz.redtorch.gateway.ctp.x64v6v3v16v.api.CThostFtdcMdSpi;
import xyz.redtorch.gateway.ctp.x64v6v3v16v.api.CThostFtdcReqUserLoginField;
import xyz.redtorch.gateway.ctp.x64v6v3v16v.api.CThostFtdcRspInfoField;
import xyz.redtorch.gateway.ctp.x64v6v3v16v.api.CThostFtdcRspUserLoginField;
import xyz.redtorch.gateway.ctp.x64v6v3v16v.api.CThostFtdcSpecificInstrumentField;
import xyz.redtorch.gateway.ctp.x64v6v3v16v.api.CThostFtdcUserLogoutField;
...
public class MdSpi extends CThostFtdcMdSpi {
	...
	public synchronized void connect() {
		...
		try {
					...
					new Thread() {
						public void run() {
							Thread.currentThread().setName("GatewayId [" + gatewayId
									+ "] MD API Release Thread, Start Time " + System.currentTimeMillis());
							try {
			...
			String tempFilePath = envTmpDir + File.separator + "xyz" + File.separator + "redtorch" + File.separator
					+ "gateway" + File.separator + "ctp" + File.separator + "jctpv6v3v16x64api" + File.separator
					+ "CTP_FLOW_TEMP" + File.separator + "MD_" + ctpGatewayImpl.getGatewayId();
			...
			new Thread() {
				public void run() {
					try {
						...
						logger.error("{}行情接口处理连接超时线程异常", logInfo, e);
					}
				}
		...
		try {
			if (cThostFtdcMdApi != null && connectionStatus != CONNECTION_STATUS_DISCONNECTING) {
				...
				try {
					if (cThostFtdcMdApi != null) {
						...
						new Thread() {
							public void run() {
								...
								Thread.currentThread().setName("GatewayId " + gatewayId
										+ " MD API Release Thread, Time " + System.currentTimeMillis());
					...
					logger.error("{}行情接口实例关闭并释放异常", logInfo, e);
				}
	...
	public boolean subscribe(String symbol) {
		...
		if (isConnected()) {
			...
			logger.warn("{}无法订阅行情,行情服务器尚未连接成功,代码:{}", logInfo, symbol);
			return false;
		}
	}
	...
	public boolean unsubscribe(String symbol) {
		...
		if (isConnected()) {
			...
			logger.warn("{}退订无效,行情服务器尚未连接成功,代码:{}", logInfo, symbol);
			return false;
		}
	}
	...
	public void OnFrontDisconnected(int nReason) {
		try {
			logger.warn("{}行情接口前置机已断开, Reason:{}", logInfo, nReason);
			ctpGatewayImpl.disconnect();
		} catch (Throwable t) {
	...
	public void OnRspUserLogin(CThostFtdcRspUserLoginField pRspUserLogin, CThostFtdcRspInfoField pRspInfo,
		...
		try {
			if (pRspInfo.getErrorID() == 0) {
				logger.info("{}OnRspUserLogin TradingDay:{},SessionID:{},BrokerId:{},UserID:{}", logInfo,
	...
	public void OnRspUserLogout(CThostFtdcUserLogoutField pUserLogout, CThostFtdcRspInfoField pRspInfo, int nRequestID,
		...
		try {
			...
			if (pRspInfo.getErrorID() != 0) {
				...
				logger.warn("{}OnRspUserLogout!BrokerId:{},UserID:{}", logInfo, pUserLogout.getBrokerID(),
	...
	public void OnRspError(CThostFtdcRspInfoField pRspInfo, int nRequestID, boolean bIsLast) {
		if (pRspInfo != null) {
			logger.error("{}行情接口错误回报!错误ID:{},错误信息:{},请求ID:{}", logInfo, pRspInfo.getErrorID(), pRspInfo.getErrorMsg(),
					nRequestID);
		} else {
	...
	public void OnRtnDepthMarketData(CThostFtdcDepthMarketDataField pDepthMarketData) {
		if (pDepthMarketData != null) {
			try {
				...
				/*
				 * 大商所获取的ActionDay可能是不正确的,因此这里采用本地时间修正
				 * 1.请注意，本地时间应该准确
				 * 2.使用 SimNow 7x24 服务器获取行情时,这个修正方式可能会导致问题
				 */
				...
				String updateDateTimeWithMS = (actionDayLong * 10000 * 1000 + updateTime * 1000 + updateMillisec) + "";
