...
public class CtpConstant {
	...
	static {
		...
		// 价格类型映射
		priceTypeMap.put(PriceTypeEnum.LIMIT, jctpv6v3v16x64apiConstants.THOST_FTDC_OPT_LimitPrice);
		priceTypeMap.put(PriceTypeEnum.MARKET, jctpv6v3v16x64apiConstants.THOST_FTDC_OPT_AnyPrice);
		priceTypeMapReverse = priceTypeMap.entrySet().stream().collect(Collectors.toMap(Map.Entry::getValue, Map.Entry::getKey));
		
		// 方向类型映射
		directionMap.put(DirectionEnum.LONG, jctpv6v3v16x64apiConstants.THOST_FTDC_D_Buy);
		directionMap.put(DirectionEnum.SHORT, jctpv6v3v16x64apiConstants.THOST_FTDC_D_Sell);
		directionMapReverse = directionMap.entrySet().stream().collect(Collectors.toMap(Map.Entry::getValue, Map.Entry::getKey));
		
		// 开平类型映射
		offsetMap.put(OffsetEnum.OPEN, jctpv6v3v16x64apiConstants.THOST_FTDC_OF_Open);
		offsetMap.put(OffsetEnum.CLOSE, jctpv6v3v16x64apiConstants.THOST_FTDC_OF_Close);
		offsetMap.put(OffsetEnum.CLOSE_TODAY, jctpv6v3v16x64apiConstants.THOST_FTDC_OF_CloseToday);
		offsetMap.put(OffsetEnum.CLOSE_YESTERDAY, jctpv6v3v16x64apiConstants.THOST_FTDC_OF_CloseYesterday);
		...
		//持仓类型映射
		posiDirectionMap.put(DirectionEnum.NET, jctpv6v3v16x64apiConstants.THOST_FTDC_PD_Net);
		posiDirectionMap.put(DirectionEnum.LONG, jctpv6v3v16x64apiConstants.THOST_FTDC_PD_Long);
		posiDirectionMap.put(DirectionEnum.SHORT, jctpv6v3v16x64apiConstants.THOST_FTDC_PD_Short);
		posiDirectionMapReverse = posiDirectionMap.entrySet().stream().collect(Collectors.toMap(Map.Entry::getValue, Map.Entry::getKey));
		

		// 产品类型映射
		productTypeMap.put(ProductTypeEnum.FUTURES, jctpv6v3v16x64apiConstants.THOST_FTDC_PC_Futures);
		productTypeMap.put(ProductTypeEnum.OPTION, jctpv6v3v16x64apiConstants.THOST_FTDC_PC_Options);
		productTypeMap.put(ProductTypeEnum.COMBINATION, jctpv6v3v16x64apiConstants.THOST_FTDC_PC_Combination);
		productTypeMap.put(ProductTypeEnum.SPOT, jctpv6v3v16x64apiConstants.THOST_FTDC_PC_Spot);
		productTypeMap.put(ProductTypeEnum.EFP, jctpv6v3v16x64apiConstants.THOST_FTDC_PC_EFP);
		productTypeMap.put(ProductTypeEnum.SPOT_OPTION, jctpv6v3v16x64apiConstants.THOST_FTDC_PC_SpotOption);
		productTypeMapReverse = productTypeMap.entrySet().stream().collect(Collectors.toMap(Map.Entry::getValue, Map.Entry::getKey));
		
		// 委托状态映射
		statusMap.put(OrderStatusEnum.ALL_TRADED, jctpv6v3v16x64apiConstants.THOST_FTDC_OST_AllTraded);
		statusMap.put(OrderStatusEnum.PART_TRADED, jctpv6v3v16x64apiConstants.THOST_FTDC_OST_PartTradedQueueing);
		statusMap.put(OrderStatusEnum.NOT_TRADED, jctpv6v3v16x64apiConstants.THOST_FTDC_OST_NoTradeQueueing);
		statusMap.put(OrderStatusEnum.CANCELLED, jctpv6v3v16x64apiConstants.THOST_FTDC_OST_Canceled);
		statusMap.put(OrderStatusEnum.UNKNOWN_ORDER_STATUS, jctpv6v3v16x64apiConstants.THOST_FTDC_OST_Unknown);
		statusMapReverse = statusMap.entrySet().stream().collect(Collectors.toMap(Map.Entry::getValue, Map.Entry::getKey));

		// 期权类型映射
		optionTypeMap.put(OptionTypeEnum.CALL, jctpv6v3v16x64apiConstants.THOST_FTDC_CP_CallOptions);
		optionTypeMap.put(OptionTypeEnum.PUT, jctpv6v3v16x64apiConstants.THOST_FTDC_CP_PutOptions);
