 ...
 */
@Service
public class TradingServiceImpl extends FastEventDynamicHandlerAbstract implements ZeusEngineService,InitializingBean {

	private Logger log = LoggerFactory.getLogger(TradingServiceImpl.class);

	private static final String logStr = "ZEUS >>> ";
	...
	public void loadStartegy() {
		...
		log.info(logContent);
		List<StrategySetting> strategySettings = zeusDataService.loadStrategySettings();
	...
	public void createStrategyClassInstance(StrategySetting strategySetting) {
		...
		try {
			...
			fastEventEngineService.addHandler(strategy);
			// Map缓存策略
	...
	public void unloadStrategy(String strategyID) {
		...
		if (strategy != null) {
			...
			fastEventEngineService.removeHandler(strategy);
			// 取消订阅合约
			for (StrategySetting.gatewaySetting gatewaySetting : strategy.getStrategySetting().getGateways()) {
				...
				for (String rtSymbol : gatewaySetting.getSubscribeRtSymbols()) {
					...
					emitInfoLog(logContent);
					log.info(logContent);
					coreEngineService.unsubscribe(rtSymbol, gatewayID, strategyID);
				}
			}
	...
	public void initStrategy(String strategyID) {
		...
		if (strategy != null) {
			...
			List<PositionDetail> tdPositionDetailList = zeusDataService.loadStrategyPositionDetails(tradingDay, strategyID,
					strategyName);
			if (tdPositionDetailList.isEmpty()) {
				...
				if (StringUtils.isEmpty(preTradingDay)) {
					...
					List<PositionDetail> ydPositionDetailList = zeusDataService.loadStrategyPositionDetails(preTradingDay,
	...
	public Contract getContract(String rtSymbol) {
		return coreEngineService.getContract(rtSymbol);
	}
	...
	public void emitErrorLog(String logContent) {
		...
		CoreUtil.emitLogBase(System.currentTimeMillis(), event, RtConstant.LOG_ERROR, logContent);
	...
	public void emitWarnLog(String logContent) {
		String event = EventConstant.EVENT_LOG + "ZEUS|";
		CoreUtil.emitLogBase(System.currentTimeMillis(), event, RtConstant.LOG_WARN, logContent);
	}

	@Override
	public void emitInfoLog(String logContent) {
		String event = EventConstant.EVENT_LOG + "ZEUS|";
		CoreUtil.emitLogBase(System.currentTimeMillis(), event, RtConstant.LOG_INFO, logContent);
	}

	@Override
	public void emitDebugLog(String logContent) {
		String event = EventConstant.EVENT_LOG + "ZEUS|";
		CoreUtil.emitLogBase(System.currentTimeMillis(), event, RtConstant.LOG_DEBUG, logContent);
	}
