 ...
 */
@Service
public class CoreEngineServiceImpl extends FastEventDynamicHandlerAbstract implements CoreEngineService,InitializingBean {
	...
	public void onEvent(final FastEvent fastEvent, final long sequence, final boolean endOfBatch) throws Exception {
		...
		if (EventConstant.EVENT_TICK.equals(fastEvent.getEventType())) {
			try {
				...
				log.error("CORE_ENGINE:onTick发生异常!!!", e);
			}
			...
			try {
				...
				log.error("CORE_ENGINE:onTrade发生异常!!!", e);
			}
			...
			try {
				...
				log.error("CORE_ENGINE:onOrder发生异常!!!", e);
			}
			...
			try {
				...
				log.error("CORE_ENGINE:onContract发生异常!!!", e);
			}
			...
			try {
				...
				log.error("CORE_ENGINE:onPosition发生异常!!!", e);
			}
			...
			try {
				...
				log.error("CORE_ENGINE:onAccount发生异常!!!", e);
			}
			...
			try {
				...
				log.error("CORE_ENGINE:onLogData发生异常!!!", e);
			}
		} else {
			log.warn("CORE_ENGINE:未能识别的事件数据类型{}", JSON.toJSONString(fastEvent.getEvent()));
		}
	}
	...
	public Contract getContract(String rtSymbol) {
		if (StringUtils.isEmpty(rtSymbol)) {
			log.error("CORE_ENGINE:查询合约不允许使用空字符串或null!!!");
			return null;
		} else {
	...
	public Contract getContract(String rtSymbol, String gatewayID) {
		if (StringUtils.isEmpty(rtSymbol)) {
			log.error("CORE_ENGINE:查询合约,rtSymbol不允许使用空字符串或null!!!");
			return null;
		} else if (StringUtils.isEmpty(gatewayID)) {
			log.error("CORE_ENGINE:查询合约,gatewayID不允许使用空字符串或null!!!");
			return null;
		} else {
	...
	public String sendOrder(OrderReq orderReq) {
		...
		if (gateway != null) {
			...
			log.error("CORE_ENGINE:发送委托失败,未能找到接口,OrderReq-{}", JSON.toJSONString(orderReq));
			return null;
		}
	}
	...
	public void queryAccount(String gatewayID) {
		...
		if (gateway != null) {
			...
			log.error("CORE_ENGINE:查询账户失败,未能找到接口{}", gatewayID);
		}
	}
	...
	public void queryPosition(String gatewayID) {
		...
		if (gateway != null) {
			...
			log.error("CORE_ENGINE:查询持仓失败,未能找到接口{}", gatewayID);
		}
	}
	...
	public void cancelOrder(CancelOrderReq cancelOrderReq) {
		...
		if (gateway != null) {
			...
			log.error("CORE_ENGINE: 撤单失败,未能找到接口,cancelOrderReq-", JSON.toJSONString(cancelOrderReq));
		}
	}
	...
	public boolean subscribe(SubscribeReq subscribeReq, String subscriberID) {
		...
		if (contract == null) {
			log.error("CORE_ENGINE:无法订阅行情,合约[{}]接口[{}]订阅者ID[{}],未找到Contract", subscribeReq.getRtSymbol(),
		...
		if (gateway != null) {
			...
			log.info("CORE_ENGINE:成功订阅行情,合约[{}]接口[{}]订阅者ID[{}]", subscribeReq.getRtSymbol(),
					subscribeReq.getGatewayID(), subscriberID);
			return true;
		} else {
			log.error("CORE_ENGINE:成功订阅行情,合约[{}]接口[{}]订阅者ID[{}]未找到接口", subscribeReq.getRtSymbol(),
					subscribeReq.getGatewayID(), subscriberID);
			return false;
		}
	}
	...
	public boolean unsubscribe(String rtSymbol, String gatewayID, String subscriberID) {
		if (StringUtils.isEmpty(rtSymbol) || StringUtils.isEmpty(gatewayID)) {
			log.error("CORE_ENGINE:无法取消订阅,参数不允许为空!");
			return false;
		}
		...
		if (subscriberIDSet == null || subscriberIDSet.isEmpty()) {
			...
			if (gateway != null) {
				...
				log.info("CORE_ENGINE:成功取消订阅行情,合约[{}]接口[{}]订阅者ID[{}]", rtSymbol, gatewayID, subscriberID);
				return true;
			} else {
				log.error("CORE_ENGINE:取消订阅行情失败,合约[{}]接口[{}]订阅者ID[{}],未找到接口", rtSymbol, gatewayID, subscriberID);
				return false;
			}
		} else {
			log.error("CORE_ENGINE:取消订阅行情失败,合约[{}]接口[{}]订阅者ID[{}],存在其它订阅者", rtSymbol, gatewayID, subscriberID);
			return false;
		}
	}
	...
	public void disconnectGateway(String gatewayID) {
		...
		if (gateway != null) {
			...
			log.error("CORE_ENGINE:接口{}不存在,无法断开!", gatewayID);
		}
	}
	...
	public List<String> scanGatewayImpl() {
		...
		if (classes == null) {
			log.error("CORE_ENGINE:未能在包xyz.redtorch.trader下扫描到任何类");
		} else {
			// 寻找Gateway的实现类,不包含抽象类
			Set<Class<?>> filteredClasses = CommonUtil.getImplementsByInterface(Gateway.class, classes, false);
			if (filteredClasses.isEmpty()) {
				log.error("CORE_ENGINE:未能在包xyz.redtorch.trader下扫描到任何Gateway接口的实现类");
			} else {
	...
	public void connectGateway(String gatewayID) {
		...
		try {
			...
			log.error("CORE_ENGINE:接口ID{},创建接口{}实例发生异常,GatewaySetting{}", gatewayID, gatewayClassName,
