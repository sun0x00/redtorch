package xyz.redtorch.core.service.extend.event;

/**
 * @author sun0x00@gmail.com
 */
public interface EventConstant {
	public final static String EVENT_TICK = "E_TICK|";
	public final static String EVENT_TICKS_CHANGED = "E_TICKS_CHANGED|";
	public final static String EVENT_TICKS = "E_TICKS|";
	public final static String EVENT_TRADE = "E_TRADE|";
	public final static String EVENT_TRADES = "E_TRADES|";
	public final static String EVENT_ORDER = "E_ORDER|";
	public final static String EVENT_ORDERS = "E_ORDERS|";
	public final static String EVENT_POSITION = "E_POSITION|";
	public final static String EVENT_POSITIONS = "E_POSITIONS|";
	public final static String EVENT_ACCOUNT = "E_ACCOUNT|";
	public final static String EVENT_ACCOUNTS = "E_ACCOUNTS|";
	public final static String EVENT_CONTRACT = "E_CONTRACT|";
	public final static String EVENT_CONTRACTS = "E_CONTRACTS|";
	public final static String EVENT_ERROR = "E_ERROR|";
	public final static String EVENT_ERRORS = "E_ERRORS|";
	public final static String EVENT_GATEWAY = "E_GATEWAY|";
	public final static String EVENT_GATEWAYS = "E_GATEWAYS|";
	public final static String EVENT_LOG = "E_LOG|";
	public final static String EVENT_LOGS = "E_LOGS|";
	public final static String EVENT_THREAD_STOP = "E_THREAD_STOP|";
	public final static String EVENT_NOTICE = "E_NOTICE|";
}
